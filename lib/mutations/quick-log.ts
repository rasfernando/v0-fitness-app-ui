...
/**
 * Saves an ad-hoc workout log for the current user.
 *
 * Creates the full chain of records so the data appears in the calendar
 * and progress graphs exactly like a scheduled workout:
 *
 *   workouts (is_adhoc, user_id = the user)
 *     → scheduled_workouts (status=completed)

...
// After removing an exercise, clear superset groups that have only one member
// left — a superset of one doesn't make sense.
function cleanLoneSupersets(exercises: FormExercise[]): FormExercise[] {
  ...
  return exercises.map((ex) => {
    if (ex.supersetGroup && groupCounts[ex.supersetGroup] === 1) {
      ...
      <div className="ml-[52px] grid grid-cols-5 gap-2">
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => onUpdate(exercise.localId, "sets", e.target.value === "" ? 0 : parseInt(e.target.value) || 0)}
            onBlur={(e) => {
...
export function PTBuilderScreen() {
  ...
  // Track which exercise is being swapped (localId), null = adding new
  const [swappingEditExerciseId, setSwappingEditExerciseId] = useState<string | null>(null)
  ...
  // activationConstraint prevents accidental drags when tapping buttons
  ...
  const handleEditDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      setEditExercises((prev) => {
        const oldIndex = prev.findIndex((e) => e.localId === active.id)
        const newIndex = prev.findIndex((e) => e.localId === over.id)
  ...
  const handleEditPickerSelect = (exercise: { id: string; name: string; category: string }) => {
    if (swappingEditExerciseId) {
      // Swap: replace exercise identity but keep everything else
      setEditExercises((prev) =>
        prev.map((e) =>
          e.localId === swappingEditExerciseId
            ? { ...e, exerciseId: exercise.id, name: exercise.name, category: exercise.category }
            : e
        )
      )
      setSwappingEditExerciseId(null)
    } else {
      // Add new exercise
      setEditExercises((prev) => [
        ...prev,
  ...
  const handleEditSwapExercise = (localId: string) => {
    setSwappingEditExerciseId(localId)
  ...
  const handleEditRemoveExercise = (localId: string) => {
    setEditExercises((prev) => {
      ...
      // Clean up lone superset members — a group of one doesn't make sense
      return cleanLoneSupersets(updated)
    })
  }
  ...
  const [swappingCreateExerciseId, setSwappingCreateExerciseId] = useState<string | null>(null)
  ...
  useBodyScrollLock(!!selectedWorkoutId || showEditPicker || showCreateWorkout || showCreatePicker)
  ...
  const handleCreatePickerSelect = (exercise: { id: string; name: string; category: string }) => {
    if (swappingCreateExerciseId) {
      // Swap: replace exercise identity but keep everything else
      setCreateExercises((prev) =>
        prev.map((e) =>
          e.localId === swappingCreateExerciseId
            ? { ...e, exerciseId: exercise.id, name: exercise.name, category: exercise.category }
            : e
        )
      )
      setSwappingCreateExerciseId(null)
    } else {
      // Add new exercise
      setCreateExercises((prev) => [
        ...prev,
  ...
  const handleCreateSwapExercise = (localId: string) => {
    setSwappingCreateExerciseId(localId)
  ...
  const handleDeleteWorkout = async (workoutId: string) => {
    try {
      ...
      if (selectedWorkoutId === workoutId) closeDetail()
      refetchWorkouts()
    } catch (err: unknown) {
                ...
                <>
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => setConfirmDeleteId(workoutDetail.id)}
              ...
              <div className="space-y-2 px-4 pb-6">
                ...
                {editing ? (
                  ...
                  <>
                    ...
                    >
                      <SortableContext
                        items={editExercises.map((e) => e.localId)}
          ...
          <div className="max-h-[85vh] overflow-y-auto rounded-t-2xl bg-card shadow-2xl scrollbar-hide safe-area-pb">
            ...
            <div className="space-y-4 p-4">
              ...
              <div>
                ...
                >
                  <SortableContext
                    items={createExercises.map((e) => e.localId)}
                    strategy={verticalListSortingStrategy}
                  >
                    <div className="space-y-3">
                      {createExercises.map((exercise, index) => (
                        <ExerciseFormRow
                          ...
                          onSwap={handleCreateSwapExercise}
                        />
                      ))}
                    </div>
                  </SortableContext>
      ...
      {confirmDeleteId && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-background/80 backdrop-blur-sm px-6">
          <div className="w-full max-w-sm rounded-2xl bg-card p-6 shadow-2xl">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              This can&apos;t be undone. Your clients&apos; logged history will be preserved.
            </p>
